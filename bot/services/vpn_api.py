...
from .panels.naive import NaiveClient
from .panels.mieru import MieruClient
...
KNOWN_PANEL_TYPES = frozenset({'xui', 'marzban', 'naive', 'mieru'})
...
def get_client_from_server_data(server: Dict[str, Any]) -> BaseVPNClient:
    ...
    None / '' / 'xui' → XUIClient.
    Известные типы → свой клиент.
    Неизвестный → VPNAPIError (без silent fallback на XUI).
    """
    server_id = server['id']
    if server_id in _clients:
        return _clients[server_id]

    raw = server.get('panel_type')
    panel_type = (raw or 'xui').strip().lower() if isinstance(raw, str) else 'xui'
    if not panel_type:
        panel_type = 'xui'

    if panel_type not in KNOWN_PANEL_TYPES:
        raise VPNAPIError(f"Неизвестный panel_type={panel_type!r} для сервера {server_id}")

    if panel_type == 'marzban':
        client: BaseVPNClient = MarzbanClient(server)
    elif panel_type == 'naive':
        client = NaiveClient(server)
    elif panel_type == 'mieru':
        client = MieruClient(server)
    else:
...
async def get_client(server_id: int) -> BaseVPNClient:
    ...
    if not server:
        raise VPNAPIError(f"Сервер {server_id} не найден")
    return get_client_from_server_data(server)


async def test_server_connection(server_data: Dict[str, Any]) -> Dict[str, Any]:
    """
    Проверяет подключение к серверу через factory (не хардкод XUI).
    """
    # Временный id для factory, если ещё не сохранён
    data = dict(server_data)
    if 'id' not in data or data['id'] is None:
        data['id'] = -abs(hash(f"{data.get('host')}:{data.get('port')}")) % (10 ** 9)

    client = None
    try:
        # Не кэшируем тестовые клиенты с отрицательным id надолго
        if data['id'] in _clients:
            del _clients[data['id']]
        client = get_client_from_server_data(data)
        await client.login()
...
async def push_key_to_panel(key_id: int, reset_traffic: bool = False) -> bool:
    ...
    inbound_id = key.get('panel_inbound_id')
    client_uuid = key.get('client_uuid')
    panel_type = (key.get('panel_type') or 'xui').strip().lower() or 'xui'

    # Marzban/naive/mieru: inbound_id может быть 0
    if not email or not client_uuid:
        logger.warning(f'push_key_to_panel: ключ {key_id} — неполные данные панели')
        return False
    if panel_type == 'xui' and not inbound_id:
        logger.warning(f'push_key_to_panel: ключ {key_id} — неполные данные панели')
        return False
    ...
    try:
        server_data = {
            ...
            'public_host': key.get('public_host'),
            'extra_config': key.get('extra_config'),
        }
        # Если panel_type не пришёл из join — подтянуть сервер
        if not key.get('panel_type'):
            from database.requests import get_server_by_id
            srv = get_server_by_id(key.get('server_id'))
            if srv:
                server_data.update({
                    'panel_type': srv.get('panel_type') or 'xui',
                    'public_host': srv.get('public_host'),
                    'extra_config': srv.get('extra_config'),
                    'protocol': srv.get('protocol', 'https'),
                    'api_token': srv.get('api_token'),
                })
                panel_type = server_data['panel_type']
        from bot.services.circuit_breaker import assert_closed, record_success, record_failure
        assert_closed(server_data['id'])
        client = get_client_from_server_data(server_data)
...
async def ensure_subscription_keys_on_server(key_id: int) -> Dict[str, int]:
    ...
    async with lock:
        ...
        panel_type = (key.get('panel_type') or 'xui').strip().lower() or 'xui'
        # XUI-путь без изменения семантики; non-xui — early return
        if panel_type != 'xui':
            logger.debug(
                f"ensure_subscription: early-return for panel_type={panel_type} key={key_id}"
            )
            return stats
