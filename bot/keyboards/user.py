...
def buy_key_kb(
    ...
    # CryptoBot — переход к выбору тарифа
    if cryptobot_enabled:
        builder.row(
            InlineKeyboardButton(text="🤖 CryptoBot (USDT/TON)", callback_data="pay_cryptobot")
        )

    # Heleket — переход к выбору тарифа
    if heleket_enabled:
        builder.row(
            InlineKeyboardButton(text="🧿 Heleket (крипта)", callback_data="pay_heleket")
        )
...
def tariff_select_kb(tariffs: list, back_callback: str = "buy_key", order_id: str = None, is_cards: bool = False, is_crypto: bool = False, is_balance: bool = False, is_qr: bool = False, groups_data: list = None, is_demo: bool = False, is_wata: bool = False, is_platega: bool = False, is_cardlink: bool = False, is_cryptobot: bool = False, is_heleket: bool = False) -> InlineKeyboardMarkup:
    """
...
def key_manage_kb(key_id: int, is_unconfigured: bool = False, is_active: bool = True, is_traffic_exhausted: bool = False, has_sub_id: bool = False, sub_page_url: str = '', auto_renew: bool = False, show_auto_renew: bool = False) -> InlineKeyboardMarkup:
    """
    ...
    Args:
        ...
        sub_page_url: URL брендированной страницы подключения (если веб-сервер включён)
        auto_renew: Текущий статус автопродления ключа
        show_auto_renew: Показывать ли кнопку автопродления
    """
    ...
    if not is_active:
        ...
        if sub_page_url:
            builder.row(
                InlineKeyboardButton(text="🌐 Страница подключения", url=sub_page_url)
            )

    if show_auto_renew and not is_unconfigured:
        ar_label = "♻️ Автопродление: ✅ вкл" if auto_renew else "♻️ Автопродление: ⚪ выкл"
        builder.row(
            InlineKeyboardButton(text=ar_label, callback_data=f"key_autorenew:{key_id}")
        )
...
def renew_tariff_select_kb(tariffs: list, key_id: int, order_id: str = None, is_cards: bool = False, is_crypto: bool = False, is_balance: bool = False, is_qr: bool = False, is_demo: bool = False, is_wata: bool = False, is_platega: bool = False, is_cardlink: bool = False, is_cryptobot: bool = False, is_heleket: bool = False) -> InlineKeyboardMarkup:
    """
...
def renew_payment_method_kb(
    ...
    if cryptobot_enabled:
        builder.row(
            InlineKeyboardButton(
                ...
                callback_data=f"renew_cryptobot_tariff:{key_id}"
            )
        )
    ...
    if heleket_enabled:
        builder.row(
            InlineKeyboardButton(
                text="🧿 Heleket (крипта)",
                callback_data=f"renew_heleket_tariff:{key_id}"
            )
        )
...
# ============================================================================
# CRYPTOBOT / HELEKET — крипто-оплата через ссылку
# ============================================================================

def cryptobot_qr_kb(order_id: str, back_callback: str = "buy_key", qr_url: str = None) -> InlineKeyboardMarkup:
    """
    Клавиатура страницы оплаты CryptoBot (Crypto Pay).

    Args:
        order_id: Наш внутренний order_id
        back_callback: Каллбэк для кнопки «Назад»
        qr_url: Ссылка на инвойс (t.me/CryptoBot?...)
    """
    builder = InlineKeyboardBuilder()

    if qr_url:
        builder.row(
            InlineKeyboardButton(text="🤖 Оплатить в CryptoBot", url=qr_url)
        )

    builder.row(
        InlineKeyboardButton(text="✅ Я оплатил", callback_data=f"check_cryptobot:{order_id}")
    )
    builder.row(
        InlineKeyboardButton(text="⬅️ Назад", callback_data=back_callback),
        InlineKeyboardButton(text="🈴 На главную", callback_data="start")
...
def heleket_qr_kb(order_id: str, back_callback: str = "buy_key", qr_url: str = None) -> InlineKeyboardMarkup:
    """
    Клавиатура страницы оплаты Heleket (крипта).
    ...
    builder.row(
        InlineKeyboardButton(text="✅ Я оплатил", callback_data=f"check_heleket:{order_id}")
    )
    builder.row(
        InlineKeyboardButton(text="⬅️ Назад", callback_data=back_callback),
        InlineKeyboardButton(text="🈴 На главную", callback_data="start")
