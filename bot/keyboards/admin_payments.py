...
def payments_menu_kb(stars_enabled: bool, crypto_enabled: bool, cards_enabled: bool, qr_enabled: bool=False, monthly_reset_enabled: bool=False, demo_enabled: bool=False, wata_enabled: bool=False, platega_enabled: bool=False, cardlink_enabled: bool=False, cryptobot_enabled: bool=False, heleket_enabled: bool=False) -> InlineKeyboardMarkup:
    ...
    builder.row(InlineKeyboardButton(text=f'🤖 CryptoBot (USDT/TON): {cryptobot_status}', callback_data='admin_payments_cryptobot'))
    heleket_status = '✅' if heleket_enabled else '❌'
    builder.row(InlineKeyboardButton(text=f'🧿 Heleket (крипта): {heleket_status}', callback_data='admin_payments_heleket'))
...
def cardlink_management_kb(is_enabled: bool) -> InlineKeyboardMarkup:
    ...
    builder.row(InlineKeyboardButton(text='🤝 Partner UUID (опц.)', callback_data='admin_cardlink_mgmt_edit_partner'))
    builder.row(back_button('admin_payments'), home_button())
    return builder.as_markup()

def cryptobot_management_kb(is_enabled: bool) -> InlineKeyboardMarkup:
    """
    Меню управления оплатой через CryptoBot (Crypto Pay API).

    Args:
        is_enabled: Включена ли оплата через CryptoBot сейчас
    """
    builder = InlineKeyboardBuilder()
    toggle_text = 'Выключить 🔴' if is_enabled else 'Включить 🟢'
...
def heleket_management_kb(is_enabled: bool) -> InlineKeyboardMarkup:
    """
    Меню управления оплатой через Heleket.

    Args:
        is_enabled: Включена ли оплата через Heleket сейчас
    """
    builder = InlineKeyboardBuilder()
    toggle_text = 'Выключить 🔴' if is_enabled else 'Включить 🟢'
    builder.row(InlineKeyboardButton(text=toggle_text, callback_data='admin_heleket_mgmt_toggle'))
    builder.row(InlineKeyboardButton(text='🆔 Изменить Merchant ID', callback_data='admin_heleket_mgmt_edit_merchant'))
    builder.row(InlineKeyboardButton(text='🔐 Изменить API-ключ', callback_data='admin_heleket_mgmt_edit_api_key'))
    builder.row(back_button('admin_payments'), home_button())
    return builder.as_markup()
