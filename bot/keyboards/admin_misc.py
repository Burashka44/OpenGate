...
def admin_main_menu_kb() -> InlineKeyboardMarkup:
    ...
    builder.row(InlineKeyboardButton(text='💝 Поддержка проекта', callback_data='admin_project_support', style='success'))
    builder.row(home_button())
    return builder.as_markup()
...
def project_support_kb(
    card_url: str = '',
    crypto_url: str = '',
    extra_url: str = '',
    extra_label: str = '',
) -> InlineKeyboardMarkup:
    """Клавиатура экрана «Поддержка проекта» — URL только из настроек."""
    builder = InlineKeyboardBuilder()
    row_buttons = []
    if card_url:
        row_buttons.append(InlineKeyboardButton(text='💳 Карты РФ', url=card_url))
    if crypto_url:
        row_buttons.append(InlineKeyboardButton(text='💰 Крипто', url=crypto_url))
    if row_buttons:
        builder.row(*row_buttons)
    if extra_url:
        label = (extra_label or 'Дополнительно')[:64]
        builder.row(InlineKeyboardButton(text=label, url=extra_url))
    builder.row(back_button('admin_panel'), home_button())
    return builder.as_markup()
