...
class AdminStates(StatesGroup):
    ...
    add_server_panel_type = State()  # Тип панели
    add_server_url = State()         # URL панели
    add_server_login = State()       # Логин
    add_server_password = State()    # Пароль
    add_server_public_host = State()
    add_server_extra_config = State()
    ...
    cardlink_setup_partner_uuid = State()  # Опциональный partner UUID Cardlink

    # ========== Настройка CryptoBot (Crypto Pay) ==========
    cryptobot_setup_token = State()     # Ввод API-токена Crypto Pay

    # ========== Настройка Heleket ==========
    heleket_setup_merchant = State()    # Ввод Merchant ID Heleket
    heleket_setup_api_key = State()     # Ввод API-ключа Heleket

    # ========== Промокоды ==========
    promo_add_input = State()           # Ввод параметров нового промокода

    # ========== UTM-кампании ==========
    campaign_add_input = State()        # Ввод кода и названия кампании

    # ========== Веб-сервер / операционные настройки ==========
    ops_setting_input = State()         # Ввод значения настройки (порт, URL, chat_id...)
...
SERVER_PARAMS = [
    ...
    },
    {
        "key": "panel_type",
        "label": "Тип панели",
        "hint": "xui | marzban | naive | mieru",
        "validate": lambda x: x.strip().lower() in ('xui', 'marzban', 'naive', 'mieru'),
        "error": "Допустимо: xui, marzban, naive, mieru"
    },
    {
        "key": "panel_url",
        "label": "URL панели",
    ...
    },
    {
        "key": "public_host",
        "label": "Public host",
        "hint": "хост для клиентских ссылок (naive/mieru), можно '-' чтобы пропустить",
    ...
    },
    {
        ...
        "hint": 'JSON настроек панели, например {"caddy_admin_url":"http://127.0.0.1:2019"} или "-"',
        "validate": lambda x: x.strip() in ('', '-') or x.strip().startswith('{'),
        "error": "Введите JSON или '-'"
    },
]
...
TARIFF_PARAMS = [
    ...
    {
        "key": "protocol_family",
        "label": "Семейство протокола",
        "hint": "xray | naive | mieru (фильтр серверов)",
        "validate": lambda x: x.strip().lower() in ('xray', 'naive', 'mieru'),
        "error": "Допустимо: xray, naive, mieru",
        "convert": lambda x: x.strip().lower(),
    },
]
...
CRYPTO_PARAMS = [
    {
        "key": "crypto_processor_url",
        "label": "Базовый URL бота процессора",
        "hint": "например: https://t.me/YourCryptoBot",
    ...
    },
    {
        ...
        "hint": "полная ссылка с ?start=item-... из вашего крипто-бота",
        "validate": lambda x: "?start=" in x and x.startswith(("http://", "https://")),
        "error": "Нужна ссылка вида https://...?start=item-..."
    },
    {
        "key": "crypto_secret_key",
        "label": "Секретный ключ подписи",
        "hint": "ключ для проверки callback bill1-...",
...
def get_crypto_param_by_key(key: str) -> dict:
    """Получает параметр крипто-настроек по ключу setting."""
