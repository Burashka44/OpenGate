...
def _build_tariff_text() -> str:
    ...
    for tariff in tariffs:
        ...
        if (cards_enabled or yookassa_qr_enabled or wata_enabled
                ...
                or cryptobot_enabled or heleket_enabled
...
async def cmd_start(message: Message, state: FSMContext, command: CommandObject):
    ...
    # UTM-кампания: /start ad_{code} — привязываем источник регистрации
    if args and args.startswith('ad_'):
        from database.requests import get_campaign_by_code, assign_user_campaign
        campaign = get_campaign_by_code(args[3:])
        if campaign:
            assign_user_campaign(user['id'], campaign['id'])
            logger.info(f"User {user_id} привязан к кампании {campaign['code']}")

    # Промокод: /start promo_{code} — сразу активируем
    if args and args.startswith('promo_'):
        from database.requests import activate_promo
        from bot.services.billing import apply_promo_effect
        (ok, msg, promo) = activate_promo(args[6:], user['id'])
        if ok and promo:
            msg = await apply_promo_effect(user['id'], promo)
        await safe_edit_or_send(message, msg, force_new=True)

    # Алерт админам о новом пользователе (если включено)
    if is_new:
        try:
            from bot.services.alerts import send_new_user_alert
            await send_new_user_alert(message.bot, user_id, username)
        except Exception as e:
            logger.debug(f'new user alert failed: {e}')
