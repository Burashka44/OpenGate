...
async def show_key_details(telegram_id: int, key_id: int, message, is_callback: bool = True, prepend_text: str=''):
    ...
    # Брендированная страница подключения (если веб-сервер настроен)
    sub_page_url = ''
    if key_active and not is_unconfigured:
        try:
            from database.requests import is_web_enabled, get_web_public_url, ensure_page_token
            public_url = get_web_public_url()
            if is_web_enabled() and public_url:
                token = ensure_page_token(key_id)
                if token:
                    sub_page_url = f"{public_url}/sub/{token}"
        except Exception as e:
            logger.warning(f'Не удалось сформировать ссылку на страницу подключения: {e}')

    # Автопродление (если включено глобально и у юзера есть баланс-функционал)
    show_auto_renew = False
    auto_renew = bool(key.get('auto_renew'))
    try:
        from database.requests import is_auto_renew_enabled
        show_auto_renew = is_auto_renew_enabled() and key_active and bool(key.get('tariff_id'))
    except Exception:
        pass

    kb = key_manage_kb(
        key_id, is_unconfigured=is_unconfigured, is_active=key_active,
        is_traffic_exhausted=traffic_exhausted, has_sub_id=bool(key.get('sub_id')),
        sub_page_url=sub_page_url, auto_renew=auto_renew, show_auto_renew=show_auto_renew,
    )
    if is_callback:
...
@router.callback_query(F.data.startswith('key_autorenew:'))
async def key_autorenew_toggle_handler(callback: CallbackQuery):
    """Переключает автопродление ключа (списание с баланса за день до истечения)."""
    from database.requests import get_key_details_for_user, set_key_auto_renew
    ...
    if not key:
        ...
        return
    new_value = not bool(key.get('auto_renew'))
    set_key_auto_renew(key_id, new_value)
    if new_value:
        await callback.answer(
            '♻️ Автопродление включено!\n\nЗа день до истечения ключ будет продлён '
            'автоматически, если на балансе хватает средств.',
            show_alert=True,
        )
    else:
...
async def key_renew_select_payment(callback: CallbackQuery):
    ...
    from database.requests import get_key_details_for_user, get_user_internal_id, is_crypto_configured, is_stars_enabled, is_cards_enabled, is_referral_enabled, get_referral_reward_type, get_user_balance, is_demo_payment_enabled, is_wata_configured, is_platega_configured, is_cardlink_configured, is_cryptobot_configured, is_heleket_configured, is_maintenance_mode
    from bot.keyboards.user import renew_payment_method_kb, back_and_home_kb
    from bot.keyboards.admin import home_only_kb
    if is_maintenance_mode():
        await safe_edit_or_send(
            callback.message,
            '🚧 <b>Технические работы</b>\n\nПродление временно недоступно.\nПопробуйте позже.',
    ...
    cryptobot_enabled = is_cryptobot_configured()
    heleket_enabled = is_heleket_configured()
    from database.requests import is_yookassa_qr_configured
    yookassa_qr = is_yookassa_qr_configured()
    if not crypto_configured and (not stars_enabled) and (not cards_enabled) and (not yookassa_qr) and (not wata_enabled) and (not platega_enabled) and (not cardlink_enabled) and (not cryptobot_enabled) and (not heleket_enabled) and (not demo_enabled):
