...
async def wata_pay_create(callback: CallbackQuery):
    ...
    from database.requests import (
        get_tariff_by_id, get_user_internal_id, create_pending_order_from_tariff, save_wata_link_id
    )
    ...
    (_, order_id) = create_pending_order_from_tariff(
        user_id=user_id, tariff=tariff, payment_type='wata', vpn_key_id=None
    )
...
async def renew_wata_create(callback: CallbackQuery):
    ...
    from database.requests import (
        get_tariff_by_id, get_user_internal_id, create_pending_order_from_tariff,
    ...
    if price_rub < 10:
        ...
        return
    (_, order_id) = create_pending_order_from_tariff(
        user_id=user_id, tariff=tariff, payment_type='wata', vpn_key_id=key_id
    )
    await safe_edit_or_send(callback.message, '⏳ Создаём ссылку на оплату...')
    try:
        bot_info = await callback.bot.get_me()
