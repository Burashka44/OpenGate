...
async def pre_checkout_handler(pre_checkout: PreCheckoutQuery):
    """Подтверждение pre-checkout для Telegram Stars / Cards."""
...
async def renew_invoice_cancel_handler(callback: CallbackQuery):
    ...
    from database.requests import get_key_details_for_user, is_crypto_configured, is_stars_enabled, is_cards_enabled, get_user_internal_id, get_setting, is_yookassa_qr_configured, is_referral_enabled, get_referral_reward_type, get_user_balance, is_demo_payment_enabled, is_wata_configured, is_platega_configured, is_cardlink_configured, is_cryptobot_configured, is_heleket_configured
    ...
    if not crypto_configured and (not stars_enabled) and (not cards_enabled) and (not yookassa_qr_enabled) and (not wata_enabled) and (not platega_enabled) and (not cardlink_enabled) and (not cryptobot_enabled) and (not heleket_enabled) and (not demo_enabled):
    ...
    await safe_edit_or_send(
        ...
        reply_markup=renew_payment_method_kb(
            ...
            heleket_enabled=heleket_enabled,
