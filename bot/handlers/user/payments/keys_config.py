...
@router.callback_query(F.data.startswith('key_configure:'))
async def key_configure_handler(callback: CallbackQuery, state: FSMContext):
    """Конфиг draft-ключа из ЛС (webhook notify) — только из user handler, не из webhook."""
    from database.requests import get_key_details_for_user, find_order_by_order_id
    ...
    if key.get('server_id'):
        await callback.answer('Ключ уже настроен', show_alert=True)
        return
    # Ищем ордер по ключу
    order_id = None
    ...
    with get_db() as conn:
        row = conn.execute(
            "SELECT order_id FROM payments WHERE vpn_key_id = ? ORDER BY id DESC LIMIT 1",
            (key_id,),
        ).fetchone()
        if row:
...
async def process_new_key_server_selection(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        # Marzban/Naive/mieru: без inbound UX (и в subscription mode)
        if not getattr(client, 'supports_inbound_select', True):
            await process_new_key_final(callback, state, server_id, 0)
            return
