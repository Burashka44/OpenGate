...
async def renew_stars_invoice(callback: CallbackQuery):
    ...
    from database.requests import get_tariff_by_id, get_user_internal_id, create_pending_order_from_tariff, get_key_details_for_user, update_order_tariff
    parts = callback.data.split(':')
    key_id = int(parts[1])
    ...
    if order_id:
        update_order_tariff(order_id, tariff_id, payment_type='stars', tariff=tariff)
    else:
        (_, order_id) = create_pending_order_from_tariff(user_id=user_id, tariff=tariff, payment_type='stars', vpn_key_id=key_id)
...
async def pay_stars_invoice(callback: CallbackQuery):
    ...
    from aiogram.types import LabeledPrice
    from database.requests import get_tariff_by_id, update_order_tariff
    parts = callback.data.split(':')
    ...
    from database.requests import get_user_internal_id, create_pending_order_from_tariff
    user_id = get_user_internal_id(callback.from_user.id)
    if not user_id:
    ...
    if order_id:
        ...
        (_, order_id) = create_pending_order_from_tariff(user_id=user_id, tariff=tariff, payment_type='stars', vpn_key_id=None)
    try:
        bot_info = await callback.bot.get_me()
        bot_name = bot_info.first_name
