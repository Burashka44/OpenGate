...
async def renew_crypto_invoice(callback: CallbackQuery):
    ...
    if order_id:
        update_order_tariff(order_id, tariff_id, payment_type='crypto', tariff=tariff)
    else:
        (_, order_id) = create_pending_order_from_tariff(user_id=user_id, tariff=tariff, payment_type='crypto', vpn_key_id=key_id)
...
async def pay_crypto_invoice(callback: CallbackQuery):
    ...
    from database.requests import get_tariff_by_id, update_order_tariff, get_setting, get_user_internal_id, create_pending_order_from_tariff
    from bot.services.billing import build_crypto_payment_url, extract_item_id_from_url
    parts = callback.data.split(':')
    ...
    if order_id:
        ...
        (_, order_id) = create_pending_order_from_tariff(user_id=user_id, tariff=tariff, payment_type='crypto', vpn_key_id=None)
    crypto_item_url = get_setting('crypto_item_url')
    item_id = extract_item_id_from_url(crypto_item_url)
    if not item_id:
        await callback.answer('❌ Ошибка настройки крипто-платежей', show_alert=True)
    ...
    price_usd = tariff['price_cents'] / 100
    price_str = f'${price_usd:g}'.replace('.', ',')
    await safe_edit_or_send(callback.message, f"💰 <b>Оплата криптовалютой</b>\n\nТариф: <b>{escape_html(tariff['name'])}</b>\nСумма к оплате: <b>{price_str}</b>\n\nНажмите кнопку ниже, чтобы перейти к оплате.", reply_markup=builder.as_markup())
    await callback.answer()