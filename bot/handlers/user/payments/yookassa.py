...
async def pay_cards_invoice(callback: CallbackQuery):
    ...
    from database.requests import get_tariff_by_id, get_user_internal_id, create_pending_order_from_tariff, update_order_tariff, get_setting
    ...
    if order_id:
        update_order_tariff(order_id, tariff_id, payment_type='cards', tariff=tariff)
    else:
        if not user_id:
            await callback.answer('❌ Ошибка пользователя', show_alert=True)
            return
        (_, order_id) = create_pending_order_from_tariff(user_id=user_id, tariff=tariff, payment_type='cards', vpn_key_id=None)
...
async def renew_cards_invoice(callback: CallbackQuery):
    ...
    from aiogram.types import LabeledPrice
    from database.requests import get_tariff_by_id, get_user_internal_id, create_pending_order_from_tariff, get_key_details_for_user, update_order_tariff, get_setting
    parts = callback.data.split(':')
    key_id = int(parts[1])
    ...
    if order_id:
        ...
        (_, order_id) = create_pending_order_from_tariff(user_id=user_id, tariff=tariff, payment_type='cards', vpn_key_id=key_id)
    price_rub = float(tariff.get('price_rub') or 0)
    price_kopecks = int(round(price_rub * 100))
    if price_kopecks <= 0:
        await callback.answer('❌ Ошибка: цена тарифа в рублях не задана.', show_alert=True)
...
async def qr_pay_create(callback: CallbackQuery):
    ...
    from database.requests import get_tariff_by_id, get_user_internal_id, create_pending_order_from_tariff, save_yookassa_payment_id
    ...
    if price_rub <= 0:
        ...
        return
    (_, order_id) = create_pending_order_from_tariff(user_id=user_id, tariff=tariff, payment_type='yookassa_qr', vpn_key_id=None)
...
async def renew_qr_create(callback: CallbackQuery):
    ...
    from database.requests import get_tariff_by_id, get_user_internal_id, create_pending_order_from_tariff, save_yookassa_payment_id, get_key_details_for_user
    from bot.services.billing import create_yookassa_qr_payment
    from bot.keyboards.user import yookassa_qr_kb
    from bot.keyboards.admin import home_only_kb
    parts = callback.data.split(':')
    ...
    if price_rub <= 0:
        ...
        return
    (_, order_id) = create_pending_order_from_tariff(user_id=user_id, tariff=tariff, payment_type='yookassa_qr', vpn_key_id=key_id)
    await safe_edit_or_send(callback.message, '⏳ Создаём QR-код для оплаты...')
    try:
        bot_info = await callback.bot.get_me()
        bot_name = bot_info.username
