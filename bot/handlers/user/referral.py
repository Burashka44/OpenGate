...
async def show_referral_system(callback: CallbackQuery):
    ...
    bot_username = getattr(callback.bot, 'my_username', None)
    if not bot_username:
    ...
    # Кнопка пополнения баланса (если настроены крипто-способы)
    append_buttons = None
    from database.requests import get_referral_reward_type, is_cryptobot_configured, is_heleket_configured
    if get_referral_reward_type() == 'balance' and (is_cryptobot_configured() or is_heleket_configured()):
