...
async def show_payments_menu(callback: CallbackQuery, state: FSMContext):
    ...
    if cryptobot:
        text += "🟢 <b>CryptoBot (USDT/TON)</b>\n"
    else:
        text += "⚪ <b>CryptoBot (USDT/TON)</b>\n"

    if heleket:
        text += "🟢 <b>Heleket (крипта)</b>\n"
    else:
        text += "⚪ <b>Heleket (крипта)</b>\n"
    ...
    await safe_edit_or_send(callback.message,
        text,
        reply_markup=payments_menu_kb(stars, crypto, cards, qr, monthly_reset, demo, wata, platega, cardlink, cryptobot, heleket)
    )
...
async def start_crypto_setup(callback: CallbackQuery, state: FSMContext):
    ...
    instructions = (
        "1️⃣ В вашем Telegram-боте процессора создайте позицию типа <b>Счёт</b>\n"
        "2️⃣ Настройте callback на этот VPN-бот (deep-link <code>bill1-...</code> + HMAC)\n\n"
        "⚠️ <b>ВАЖНО:</b>\n"
        "• Тип позиции — <b>Счёт</b>, не «Товар»\n"
        "• Тарифы к позиции добавлять не нужно — сумму задаёт OpenGate\n\n"
    )

    text = (
        "💰 <b>Настройка крипто-платежей</b>\n\n"
        "Подключите совместимый крипто-процессор (Telegram-бот с форматом "
        "<code>?start=item-...</code> и callback <code>bill...</code>).\n\n"
        f"{instructions}"
        "🔗 <b>Отправьте полную ссылку на позицию</b> (с <code>?start=item-...</code>):"
    )
...
async def process_crypto_url(message: Message, state: FSMContext):
    ...
    # Валидация
    param = get_crypto_param_by_key('crypto_item_url')
    ...
    if edit_mode:
        ...
        if '?start=' in url:
            set_setting('crypto_processor_url', url.split('?start=')[0])
        ...
        await safe_edit_or_send(message,
            ...
            "В настройках позиции процессора укажите обратную ссылку (callback) на этот бот:\n"
            f"<code>{callback_url}</code>\n\n"
            "🔑 <b>Ожидаю секретный ключ подписи</b> для проверки callback <code>bill1-...</code>:",
...
async def process_crypto_secret(message: Message, state: FSMContext):
    ...
    # Валидация
    param = get_crypto_param_by_key('crypto_secret_key')
...
async def crypto_setup_save(callback: CallbackQuery, state: FSMContext):
    ...
    # Сохраняем
    item_url = crypto_data['crypto_item_url']
    set_setting('crypto_item_url', item_url)
    set_setting('crypto_secret_key', crypto_data['crypto_secret_key'])
    if '?start=' in item_url:
        set_setting('crypto_processor_url', item_url.split('?start=')[0])
...
async def crypto_mgmt_edit_url(callback: CallbackQuery, state: FSMContext):
    ...
    instructions = (
        "1️⃣ В боте процессора создайте позицию типа <b>Счёт</b>\n"
        "2️⃣ Скопируйте полную ссылку с <code>?start=item-...</code>\n\n"
        "⚠️ Тарифы к позиции не добавляйте — сумму задаёт OpenGate.\n\n"
    )
    ...
    if current_url:
        ...
        text = (
            ...
            "🔗 <b>Введите новую ссылку на позицию:</b>"
        )
    else:
        text = (
            "🔗 <b>Изменение ссылки</b>\n\n"
...
async def crypto_mgmt_edit_secret(callback: CallbackQuery, state: FSMContext):
    ...
    text = (
        ...
        "🔔 Укажите callback (обратную ссылку) на этот бот в настройках позиции:\n"
        f"<code>{callback_url}</code>\n\n"
        "🔑 <b>Отправьте новый секретный ключ подписи</b> для callback <code>bill1-...</code>:"
    )
...
async def show_cards_management_menu(callback: CallbackQuery, state: FSMContext):
    ...
    text = (
        ...
        "Зарегистрируйте магазин на <a href=\"https://yookassa.ru/\">yookassa.ru</a>\n\n"
...
async def cards_mgmt_edit_token(callback: CallbackQuery, state: FSMContext):
    ...
    text = (
        ...
        "Зарегистрируйтесь на <a href=\"https://yookassa.ru/\">yookassa.ru</a>\n\n"
...
async def show_qr_management_menu(callback: CallbackQuery, state: FSMContext):
    ...
    text = (
        ...
        "1. Зарегистрируйте магазин: <a href=\"https://yookassa.ru/\">yookassa.ru</a>\n"
...
async def show_wata_management_menu(callback: CallbackQuery, state: FSMContext):
    ...
    text = (
        ...
        "2. Завершите регистрацию и получите JWT-токен в личном кабинете.\n"
        "3. После регистрации получите JWT-токен в личном кабинете WATA.\n"
...
async def show_platega_management_menu(callback: CallbackQuery, state: FSMContext):
    ...
    text = (
        ...
        "Минимальная сумма платежа: <b>10 ₽</b>.\n\n"
        "📋 <b>Как получить доступ:</b>\n"
        "1. Зарегистрируйтесь на <a href=\"https://app.platega.io/\">app.platega.io</a>\n"
        "2. После подключения скопируйте <b>Merchant ID</b> и <b>Secret</b> из личного кабинета.\n"
...
async def show_cardlink_management_menu(callback: CallbackQuery, state: FSMContext):
    ...
    partner_uuid = get_setting('cardlink_partner_uuid', '') or ''
    ...
    if partner_uuid:
        if len(partner_uuid) >= 12:
            partner_display = f"Установлен ✅ (<code>{escape_html(partner_uuid[:4])}...{escape_html(partner_uuid[-4:])}</code>)"
        else:
            partner_display = "Установлен ✅"
    else:
        partner_display = "— (не задан, опционально)"
    ...
    text = (
        ...
        f"🤝 Partner UUID: {partner_display}\n\n"
        "Выберите действие:"
    )
...
@router.callback_query(F.data == "admin_cardlink_mgmt_edit_partner")
async def cardlink_edit_partner(callback: CallbackQuery, state: FSMContext):
    """Редактирование опционального partner UUID Cardlink."""
    if not is_admin(callback.from_user.id):
        await callback.answer("⛔ Доступ запрещён", show_alert=True)
        return

    await state.set_state(AdminStates.cardlink_setup_partner_uuid)
    current = get_setting('cardlink_partner_uuid', '') or '—'
    await safe_edit_or_send(
        callback.message,
        "🤝 <b>Partner UUID Cardlink</b> (опционально)\n\n"
        f"Текущее: <code>{escape_html(current)}</code>\n\n"
        "Отправьте UUID из партнёрской программы Cardlink или <code>-</code> чтобы очистить:",
        reply_markup=back_and_home_kb("admin_payments_cardlink"),
    )
    await callback.answer()


@router.message(AdminStates.cardlink_setup_partner_uuid)
async def cardlink_setup_partner_uuid_handler(message: Message, state: FSMContext):
    ...
    if value == '-':
        delete_setting('cardlink_partner_uuid')
        saved_text = "✅ Partner UUID удалён"
    else:
        set_setting('cardlink_partner_uuid', value)
        saved_text = "✅ Partner UUID сохранён"
    ...
    await safe_edit_or_send(message, saved_text, force_new=True)
...
# ============================================================================
# НАСТРОЙКА CRYPTOBOT (Crypto Pay API)
# ============================================================================

class _FakeCallback:
    """Обёртка Message → CallbackQuery для перерисовки меню после ввода."""
    def __init__(self, msg, user):
        self.message = msg
        self.from_user = user
        self.bot = msg.bot
...
async def _redraw_menu_after_input(message: Message, state: FSMContext, menu_func):
    """Удаляет ввод пользователя и перерисовывает меню провайдера."""
    try:
        await message.delete()
    except Exception:
        pass
    ...
    fake = _FakeCallback(menu_message, message.from_user)
    await menu_func(fake, state)
...
async def show_cryptobot_management_menu(callback: CallbackQuery, state: FSMContext):
    """Показывает меню управления оплатой через CryptoBot."""
    if not is_admin(callback.from_user.id):
        await callback.answer("⛔ Доступ запрещён", show_alert=True)
        return
    ...
    token = get_setting('cryptobot_token', '') or ''
    ...
    from database.requests import get_web_public_url
    webhook_url = (get_web_public_url() or 'https://ваш-домен') + '/webhook/cryptobot'

    text = (
        "🤖 <b>Оплата CryptoBot (Crypto Pay API)</b>\n\n"
        "Приём криптовалюты (USDT, TON, BTC и др.) через официальный "
        "@CryptoBot. Сумма фиксируется в рублях, конвертация — по курсу CryptoBot.\n\n"
        "📋 <b>Как получить токен:</b>\n"
        "1. Откройте <a href=\"https://t.me/CryptoBot\">@CryptoBot</a> → Crypto Pay\n"
        "2. Создайте приложение (Create App)\n"
        "3. Скопируйте API Token и вставьте сюда (кнопка ниже)\n\n"
        f"💡 Вебхук для мгновенного зачисления: <code>{webhook_url}</code>\n\n"
        f"{status_emoji} Статус: <b>{status_text}</b>\n"
        f"🔑 API-токен: {token_display}\n\n"
        "Выберите действие:"
    )
    ...
    await safe_edit_or_send(
        callback.message,
        text,
        reply_markup=cryptobot_management_kb(is_enabled),
    )
    await callback.answer()
...
async def cryptobot_mgmt_toggle(callback: CallbackQuery, state: FSMContext):
    """Включает/выключает оплату через CryptoBot."""
    if not is_admin(callback.from_user.id):
        await callback.answer("⛔ Доступ запрещён", show_alert=True)
        return
    ...
    if not current:
        ...
        if not token or not token.strip():
            await callback.answer("❌ Сначала укажите API-токен!", show_alert=True)
            return
    ...
    status = "включена ✅" if new_value == '1' else "выключена"
    await callback.answer(f"CryptoBot-оплата {status}")
...
async def cryptobot_edit_token(callback: CallbackQuery, state: FSMContext):
    """Запрашивает API-токен Crypto Pay."""
    if not is_admin(callback.from_user.id):
        await callback.answer("⛔ Доступ запрещён", show_alert=True)
        return
    ...
    await safe_edit_or_send(
        ...
        "Получите в <a href=\"https://t.me/CryptoBot\">@CryptoBot</a>: "
        "<b>Crypto Pay → My Apps → API Token</b>.\n\n"
        "<i>Токен будет частично скрыт после сохранения.</i>",
...
async def cryptobot_setup_token_handler(message: Message, state: FSMContext):
    """Обрабатывает ввод API-токена Crypto Pay."""
    from bot.utils.text import get_message_text_for_storage
    ...
    set_setting('cryptobot_token', token)
    await _redraw_menu_after_input(message, state, show_cryptobot_management_menu)
...
@router.callback_query(F.data == "admin_payments_heleket")
async def show_heleket_management_menu(callback: CallbackQuery, state: FSMContext):
    """Показывает меню управления оплатой через Heleket."""
    if not is_admin(callback.from_user.id):
        await callback.answer("⛔ Доступ запрещён", show_alert=True)
        return
    ...
    is_enabled = is_heleket_enabled()
    merchant_id = get_setting('heleket_merchant_id', '') or ''
    api_key = get_setting('heleket_api_key', '') or ''
    ...
    if api_key:
        if len(api_key) >= 12:
            key_display = f"Установлен ✅ (<code>{escape_html(api_key[:4])}...{escape_html(api_key[-4:])}</code>)"
        else:
            key_display = "Установлен ✅"
    else:
        key_display = "❌ Не задан"

    from database.requests import get_web_public_url
    webhook_url = (get_web_public_url() or 'https://ваш-домен') + '/webhook/heleket'

    text = (
        "🧿 <b>Оплата Heleket (криптовалюта)</b>\n\n"
        "Крипто-эквайринг: USDT, TON, BTC, ETH и десятки других монет. "
        "Сумма фиксируется в рублях.\n\n"
        "📋 <b>Как получить доступ:</b>\n"
        "1. Зарегистрируйтесь: <a href=\"https://heleket.com\">heleket.com</a>\n"
        "2. Создайте мерчанта и получите <b>Merchant ID</b> и <b>Payment API key</b>\n"
        "3. Вставьте их сюда (кнопки ниже)\n\n"
        f"💡 Вебхук для мгновенного зачисления: <code>{webhook_url}</code>\n\n"
        f"{status_emoji} Статус: <b>{status_text}</b>\n"
        f"🆔 Merchant ID: {merchant_display}\n"
        f"🔐 API-ключ: {key_display}\n\n"
        "Выберите действие:"
    )
    ...
    await safe_edit_or_send(
        callback.message,
        text,
        reply_markup=heleket_management_kb(is_enabled),
    )
    await callback.answer()


@router.callback_query(F.data == "admin_heleket_mgmt_toggle")
async def heleket_mgmt_toggle(callback: CallbackQuery, state: FSMContext):
    """Включает/выключает оплату через Heleket."""
    if not is_admin(callback.from_user.id):
        await callback.answer("⛔ Доступ запрещён", show_alert=True)
        return

    current = is_heleket_enabled()
    if not current:
        merchant_id = get_setting('heleket_merchant_id', '')
        api_key = get_setting('heleket_api_key', '')
        if not (merchant_id and merchant_id.strip() and api_key and api_key.strip()):
            await callback.answer("❌ Сначала укажите Merchant ID и API-ключ!", show_alert=True)
            return

    new_value = '0' if current else '1'
    set_setting('heleket_enabled', new_value)

    status = "включена ✅" if new_value == '1' else "выключена"
    await callback.answer(f"Heleket-оплата {status}")
    await show_heleket_management_menu(callback, state)


@router.callback_query(F.data == "admin_heleket_mgmt_edit_merchant")
async def heleket_edit_merchant(callback: CallbackQuery, state: FSMContext):
    """Запрашивает Merchant ID Heleket."""
    if not is_admin(callback.from_user.id):
        await callback.answer("⛔ Доступ запрещён", show_alert=True)
        return

    await state.set_state(AdminStates.heleket_setup_merchant)
    await state.update_data(last_menu_msg_id=callback.message.message_id)

    await safe_edit_or_send(
        callback.message,
        "🆔 <b>Введите Merchant ID Heleket</b>\n\n"
        "Найдите в личном кабинете <a href=\"https://heleket.com\">heleket.com</a>: "
        "<b>Merchants → ваш мерчант → UUID</b>.",
        reply_markup=back_and_home_kb("admin_payments_heleket"),
    )
    await callback.answer()


@router.message(AdminStates.heleket_setup_merchant)
async def heleket_setup_merchant_handler(message: Message, state: FSMContext):
    """Обрабатывает ввод Merchant ID Heleket."""
    from bot.utils.text import get_message_text_for_storage
    ...
    set_setting('heleket_merchant_id', merchant_id)
    await _redraw_menu_after_input(message, state, show_heleket_management_menu)


@router.callback_query(F.data == "admin_heleket_mgmt_edit_api_key")
async def heleket_edit_api_key(callback: CallbackQuery, state: FSMContext):
    """Запрашивает API-ключ Heleket."""
    if not is_admin(callback.from_user.id):
        await callback.answer("⛔ Доступ запрещён", show_alert=True)
        return

    await state.set_state(AdminStates.heleket_setup_api_key)
    await state.update_data(last_menu_msg_id=callback.message.message_id)

    await safe_edit_or_send(
        callback.message,
        "🔐 <b>Введите Payment API key Heleket</b>\n\n"
        "Сгенерируйте в личном кабинете: <b>Settings → API keys → Payment</b>.\n\n"
        "<i>Ключ будет частично скрыт после сохранения.</i>",
        reply_markup=back_and_home_kb("admin_payments_heleket"),
    )
    await callback.answer()


@router.message(AdminStates.heleket_setup_api_key)
async def heleket_setup_api_key_handler(message: Message, state: FSMContext):
    """Обрабатывает ввод API-ключа Heleket."""
    from bot.utils.text import get_message_text_for_storage
    ...
    set_setting('heleket_api_key', api_key)
    await _redraw_menu_after_input(message, state, show_heleket_management_menu)
