...
ADD_STATES = [
    ...
    AdminStates.add_server_panel_type,
    AdminStates.add_server_url,
    AdminStates.add_server_login,
    AdminStates.add_server_password,
    AdminStates.add_server_public_host,
    AdminStates.add_server_extra_config,
]
...
async def process_add_step(message: Message, state: FSMContext):
    ...
    if param['key'] == 'panel_url':
        ...
        if param['key'] == 'panel_type':
            value = value.strip().lower()
        if param['key'] in ('public_host', 'extra_config') and value.strip() == '-':
            value = ''
        if param['key'] == 'extra_config' and value.strip() == '':
            value = '{}'
    ...
    if current_step < get_total_params():
        ...
        # Тестируем подключение (обязательно для выбранного panel_type)
        if not server_data.get('panel_type'):
            server_data['panel_type'] = 'xui'
        test_result = await test_server_connection(server_data)
        
        if test_result['success']:
            stats = test_result.get('stats', {}) or {}
            traffic = format_traffic(stats.get('total_traffic_bytes', 0) or 0)
            
            text = (
                f"✅ <b>Проверка подключения успешна!</b>\n\n"
                f"📦 Тип: <code>{server_data.get('panel_type', 'xui')}</code>\n"
                f"📊 Статистика:\n"
                f"   🔑 Онлайн: {stats.get('online_clients', 0) or stats.get('online', 0)}\n"
                f"   📈 Трафик: {traffic}\n\n"
                f"Сохранить сервер?"
            )
            kb = add_server_confirm_kb()
            await state.update_data(connection_ok=True)
        else:
            text = (
                f"❌ <b>Ошибка подключения</b>\n\n"
                f"<code>{test_result['message']}</code>\n\n"
                f"Сохранение без успешного теста запрещено. Исправьте данные и повторите."
            )
            kb = add_server_test_failed_kb()
            await state.update_data(connection_ok=False)
...
@router.message(AdminStates.add_server_panel_type)
...
@router.message(AdminStates.add_server_public_host)
@router.message(AdminStates.add_server_extra_config)
async def add_server_step_handler(message: Message, state: FSMContext):
...
async def add_server_retest(callback: CallbackQuery, state: FSMContext):
    ...
    if test_result['success']:
        ...
        text = (
            f"❌ <b>Ошибка подключения</b>\n\n"
            f"<code>{test_result['message']}</code>\n\n"
            f"Сохранение без успешного теста запрещено."
        )
        kb = add_server_test_failed_kb()
        await state.update_data(connection_ok=False)
...
async def add_server_save(callback: CallbackQuery, state: FSMContext):
    ...
    data = await state.get_data()
    server_data = data.get('server_data', {})
    if not data.get('connection_ok'):
        # Обязательный успешный тест перед save
        test_result = await test_server_connection(server_data)
        if not test_result['success']:
            await callback.answer(
                f"❌ Сначала успешный тест: {test_result['message'][:100]}",
                show_alert=True,
            )
            return
        await state.update_data(connection_ok=True)
    ...
    try:
        server_id = add_server(
            ...
            panel_type=server_data.get('panel_type') or 'xui',
            public_host=server_data.get('public_host') or None,
            extra_config=server_data.get('extra_config') or None,
        )
