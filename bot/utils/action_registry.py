...
def _resolve_pay_cryptobot(ctx: dict) -> Optional[dict]:
    """Кнопка оплаты через CryptoBot (Crypto Pay)."""
...
def _resolve_pay_heleket(ctx: dict) -> Optional[dict]:
    """Кнопка оплаты через Heleket (крипта)."""
    from database.requests import is_heleket_configured

    if not is_heleket_configured():
        return None

    return {"callback_data": "pay_heleket"}
...
SYSTEM_BUTTONS: Dict[str, Callable[[dict], Optional[dict]]] = {
    ...
    "btn_pay_heleket": _resolve_pay_heleket,
