    ...
    with get_db() as conn:
        cursor = conn.execute("""
            SELECT id, name, host, port, web_base_path, login, password, is_active, protocol, api_token,
                   panel_type, weight, public_host, extra_config
            FROM servers
            ORDER BY id
        """)
...
def add_server(
    ...
    """
    panel_type = (panel_type or 'xui').strip().lower() or 'xui'
    with get_db() as conn:
        cursor = conn.execute("""
            INSERT INTO servers (
                name, host, port, web_base_path, login, password, is_active, protocol,
                panel_type, public_host, extra_config
            )
            VALUES (?, ?, ?, ?, ?, ?, 1, ?, ?, ?, ?)
        """, (
        ...
        logger.info(f"Добавлен сервер: {name} (ID: {server_id}, panel_type={panel_type}, группа: {group_id})")
        return server_id
