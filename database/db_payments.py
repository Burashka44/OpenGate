...
__all__ = [
    ...
    'mark_referral_paid',
    'mark_user_notified',
    'claim_balance_to_deduct',
    'save_provider_invoice_id',
    'find_order_by_provider_invoice_id',
    'create_pending_topup_order',
    'get_key_payments_history',
    'get_referral_levels',
...
# Типы оплаты, где amount_cents хранит копейки RUB (не USD cents).
RUB_AMOUNT_PAYMENT_TYPES = frozenset({
    'cards', 'yookassa_qr', 'wata', 'platega', 'cardlink',
    'cryptobot', 'heleket', 'balance',
})


def charged_amounts_from_tariff(
    tariff: Optional[Dict[str, Any]],
    payment_type: Optional[str],
) -> Tuple[int, int, Optional[int]]:
    """
    Фактические суммы ордера из (возможно скидочного) тарифа.

    Returns:
        (amount_cents, amount_stars, period_days)
        amount_cents: USD cents для crypto/stars; копейки RUB для rub-провайдеров.
    """
    if not tariff:
        return 0, 0, None
    period = tariff.get('duration_days')
    stars = int(tariff.get('price_stars') or 0)
    if payment_type in RUB_AMOUNT_PAYMENT_TYPES:
        cents = int(round(float(tariff.get('price_rub') or 0) * 100))
    else:
        cents = int(tariff.get('price_cents') or 0)
    return cents, stars, period


def create_pending_order_from_tariff(
    user_id: int,
    tariff: Dict[str, Any],
    payment_type: Optional[str],
    ...
    """
    Создаёт pending order с суммами из переданного тарифа (уже со скидкой).

    Handlers должны передавать результат get_discounted_tariff, а не сырой тариф из БД.
    """
    tariff_id = tariff.get('id')
    amount_cents, amount_stars, period_days = charged_amounts_from_tariff(tariff, payment_type)
    # Partial balance: в amount_cents пишем оставшуюся к оплате сумму (если доплата).
    remaining_cents = amount_cents
    if balance_to_deduct and payment_type in RUB_AMOUNT_PAYMENT_TYPES:
        remaining_cents = max(0, amount_cents - int(balance_to_deduct))

    with get_db() as conn:
        cursor = conn.execute("""
            INSERT INTO payments
            (user_id, tariff_id, order_id, payment_type, vpn_key_id,
             amount_cents, amount_stars, period_days, status, paid_at, balance_to_deduct)
            VALUES (?, ?, 'pending', ?, ?, ?, ?, ?, 'pending', NULL, ?)
        """, (
            user_id, tariff_id, payment_type, vpn_key_id,
            remaining_cents if balance_to_deduct else amount_cents,
            amount_stars,
            period_days,
            int(balance_to_deduct or 0),
        ))
        payment_id = cursor.lastrowid
        order_id = "00" + _int_to_base62(payment_id)
        conn.execute("UPDATE payments SET order_id = ? WHERE id = ?", (order_id, payment_id))
        logger.info(
            f"Создан pending order: {order_id} (id={payment_id}, user={user_id}, "
            f"type={payment_type}, amount_cents={remaining_cents if balance_to_deduct else amount_cents}, "
            f"balance_to_deduct={balance_to_deduct})"
        )
...
def create_pending_order(
    ...
    """
    Создаёт pending order из тарифа БД (без персональной скидки).

    Предпочтительно использовать create_pending_order_from_tariff после get_discounted_tariff.
    """
    tariff = get_tariff_by_id(tariff_id) if tariff_id else None
    if tariff:
        return create_pending_order_from_tariff(
            user_id=user_id,
            tariff=tariff,
    ...
    with get_db() as conn:
        cursor = conn.execute("""
            ...
            (user_id, tariff_id, order_id, payment_type, vpn_key_id,
             amount_cents, amount_stars, period_days, status, paid_at, balance_to_deduct)
            VALUES (?, ?, 'pending', ?, ?, 0, 0, NULL, 'pending', NULL, ?)
        """, (user_id, tariff_id, payment_type, vpn_key_id, int(balance_to_deduct or 0)))
        payment_id = cursor.lastrowid
        order_id = "00" + _int_to_base62(payment_id)
        conn.execute("UPDATE payments SET order_id = ? WHERE id = ?", (order_id, payment_id))
...
def update_order_tariff(
    order_id: str,
    tariff_id: int,
    payment_type: Optional[str] = None,
    tariff: Optional[Dict[str, Any]] = None,
    balance_to_deduct: Optional[int] = None,
) -> bool:
    """
    Обновляет тариф и суммы в ордере.

    Если передан tariff (скидочный) — суммы берутся из него.
    """
    tariff = tariff or get_tariff_by_id(tariff_id)
    if not tariff:
        return False

    with get_db() as conn:
        row = conn.execute(
            "SELECT payment_type, balance_to_deduct FROM payments WHERE order_id = ?",
            (order_id,)
        ).fetchone()
        if not row:
            return False
        effective_type = payment_type or row['payment_type']
        amount_cents, amount_stars, period_days = charged_amounts_from_tariff(tariff, effective_type)
        bal = int(balance_to_deduct) if balance_to_deduct is not None else int(row['balance_to_deduct'] or 0)
        if bal and effective_type in RUB_AMOUNT_PAYMENT_TYPES:
            amount_cents = max(0, amount_cents - bal)
...
def mark_order_fulfilled(order_id: str) -> None:
    """Фиксирует момент фактической выдачи (продление/создание ключа)."""
    with get_db() as conn:
        conn.execute(
            "UPDATE payments SET fulfilled_at = CURRENT_TIMESTAMP WHERE order_id = ?",
            (order_id,)
        )


def mark_referral_paid(order_id: str) -> bool:
    """Ставит referral_paid_at, если ещё не стоял. True = только что отметили."""
    with get_db() as conn:
        cursor = conn.execute("""
            UPDATE payments
            SET referral_paid_at = CURRENT_TIMESTAMP
            WHERE order_id = ? AND referral_paid_at IS NULL
        """, (order_id,))
        return cursor.rowcount > 0


def mark_user_notified(order_id: str) -> bool:
    """Ставит user_notified_at, если ещё не стоял. True = только что отметили."""
    with get_db() as conn:
        cursor = conn.execute("""
            UPDATE payments
            SET user_notified_at = CURRENT_TIMESTAMP
            WHERE order_id = ? AND user_notified_at IS NULL
        """, (order_id,))
        return cursor.rowcount > 0


def claim_balance_to_deduct(order_id: str) -> int:
    """
    Атомарно забирает balance_to_deduct (ставит 0).
    Возвращает сумму к списанию или 0, если уже забрали / нечего списывать.
    """
    with get_db() as conn:
        row = conn.execute(
            "SELECT balance_to_deduct FROM payments WHERE order_id = ?",
            (order_id,),
        ).fetchone()
        if not row:
            return 0
        bal = int(row['balance_to_deduct'] or 0)
        if bal <= 0:
            return 0
        cursor = conn.execute(
            """
            UPDATE payments
            SET balance_to_deduct = 0
            WHERE order_id = ? AND balance_to_deduct = ?
            """,
            (order_id, bal),
        )
...
def save_provider_invoice_id(order_id: str, invoice_id: str) -> bool:
    """
    Сохраняет универсальный ID инвойса провайдера (CryptoBot, Heleket).

    Args:
        order_id: Наш внутренний order_id
        invoice_id: ID инвойса в системе провайдера
    """
    with get_db() as conn:
        cursor = conn.execute(
            "UPDATE payments SET provider_invoice_id = ? WHERE order_id = ?",
            (invoice_id, order_id)
        )
        success = cursor.rowcount > 0
        if success:
            logger.info(f"Сохранён provider_invoice_id={invoice_id} для order_id={order_id}")
        return success


def find_order_by_provider_invoice_id(invoice_id: str) -> Optional[Dict[str, Any]]:
    """Находит ордер по ID инвойса провайдера (CryptoBot, Heleket)."""
    with get_db() as conn:
        cursor = conn.execute(
...
def create_pending_topup_order(user_id: int, amount_cents: int, payment_type: str) -> tuple[int, str]:
    """
    Создаёт pending order пополнения баланса (purpose='topup').

    Args:
        user_id: Внутренний ID пользователя
        amount_cents: Сумма пополнения в копейках
    ...
    with get_db() as conn:
        cursor = conn.execute("""
            INSERT INTO payments
            (user_id, tariff_id, order_id, payment_type, vpn_key_id,
             amount_cents, amount_stars, period_days, status, paid_at, purpose)
            VALUES (?, NULL, 'pending', ?, NULL, ?, 0, NULL, 'pending', NULL, 'topup')
        """, (user_id, payment_type, amount_cents))
        payment_id = cursor.lastrowid
        order_id = "00" + _int_to_base62(payment_id)
        conn.execute("UPDATE payments SET order_id = ? WHERE id = ?", (order_id, payment_id))
        logger.info(f"Создан pending topup order: {order_id} (user={user_id}, {amount_cents} коп)")
        return payment_id, order_id
