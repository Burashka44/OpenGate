    ...
    page_defaults = {
        ...
        'help': {
            'text': (
                ...
                "1. Купите ключ в разделе «Купить ключ» или откройте уже купленный в «Мои ключи».\n"
                "2. Нажмите «Получить ключ» и скопируйте ссылку (обычно начинается с <code>vless://</code> или похожей).\n"
                "3. Установите клиент для вашего устройства:\n"
                "• <b>iPhone / iPad</b> — V2Box или Streisand\n"
                "• <b>Android</b> — v2RayTun, V2Box или Hiddify\n"
                "• <b>Windows / Mac / Linux</b> — Hiddify\n"
                "4. В приложении импортируйте ключ из буфера обмена и нажмите Connect / Подключить.\n\n"
                "<b>Важно:</b> 1 ключ = 1 активное устройство одновременно. "
                "Если нужно два устройства сразу — купите второй ключ.\n\n"
                "Если что-то не открывается после подключения, переключите клиент "
                "из режима Proxy в VPN / TUN.\n\n"
                "Вопросы — кнопка «Поддержка» ниже (настраивается в админке)."
            ),
            'buttons': json.dumps([
                {"id": "btn_news",      "label": "📢 Новости",    "color": "secondary", "row": 0, "col": 0, "is_hidden": True, "action_type": "url", "action_value": ""},
                {"id": "btn_support",   "label": "💬 Поддержка",  "color": "secondary", "row": 0, "col": 1, "is_hidden": True, "action_type": "url", "action_value": ""},
...
def migration_30(conn):
    """
    Миграция v30: фундамент для веб-сервера, мульти-панелей и retention-фич.

    - servers.panel_type — тип панели: 'xui' | 'marzban' | 'naive' | 'mieru'
    - servers.weight — вес для least-load выбора сервера
    - servers.public_host — адрес для клиентских конфигов (naive/mieru)
    - servers.extra_config — JSON c панель-специфичными настройками
    - tariffs.protocol_family — 'xray' | 'naive' | 'mieru' (фильтрация серверов)
    - vpn_keys.page_token — токен subscription page (/sub/{token})
    - vpn_keys.auto_renew — автопродление с баланса
    - vpn_keys.access_password — пароль пользователя для naive/mieru
    - vpn_keys.link_reset_at — последний сброс ссылки (cooldown)
    - payments.fulfilled_at — момент фактической выдачи (диагностика)
    - payments.purpose — 'purchase' | 'topup' (пополнение баланса)
    - настройки веб-сервера, maintenance, алертов и trial-гейта
    """
    _add_column(conn, "servers", "panel_type TEXT DEFAULT 'xui'")
    _add_column(conn, "servers", "weight REAL DEFAULT 1.0")
    _add_column(conn, "servers", "public_host TEXT")
    _add_column(conn, "servers", "extra_config TEXT")

    _add_column(conn, "tariffs", "protocol_family TEXT DEFAULT 'xray'")

    _add_column(conn, "vpn_keys", "page_token TEXT")
    _add_column(conn, "vpn_keys", "auto_renew INTEGER DEFAULT 0")
    _add_column(conn, "vpn_keys", "access_password TEXT")
    _add_column(conn, "vpn_keys", "link_reset_at DATETIME")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_vpn_keys_page_token ON vpn_keys(page_token)")

    _add_column(conn, "payments", "fulfilled_at DATETIME")
    _add_column(conn, "payments", "purpose TEXT DEFAULT 'purchase'")

    web_settings = [
        ('web_enabled', '0'),
        ('web_port', '8081'),
        ('web_public_url', ''),          # например https://sub.example.com
        ('maintenance_mode', '0'),
        ('healthcheck_enabled', '0'),
        ('healthcheck_auto_maintenance', '1'),
        ('admin_alerts_chat_id', ''),    # чат/канал для алертов (пусто = админам в ЛС)
        ('admin_alerts_payments', '1'),
        ('admin_alerts_new_users', '0'),
        ('trial_channel_id', ''),        # обязательная подписка на канал для trial
        ('trial_channel_link', ''),
        ('auto_renew_enabled', '0'),
        ('link_reset_cooldown_hours', '24'),
    ]
    for key, value in web_settings:
        conn.execute("INSERT OR IGNORE INTO settings (key, value) VALUES (?, ?)", (key, value))

    logger.info("Миграция v30 применена: panel_type/weight, page_token/auto_renew, purpose, веб-настройки")


def migration_31(conn):
    """
    Миграция v31: промокоды.

    - promo_codes: code, type ('days'|'percent'|'balance'|'trial'), value,
      max_uses, per_user_limit, expires_at, is_active
    - promo_activations: журнал активаций (кто, когда, каким ордером)
    - users.next_discount_percent — персональная скидка на следующую покупку
      (устанавливается промокодом типа 'percent', сгорает при оплате)
    """
    conn.execute("""
    ...
    """)
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_promo_activations_promo_user "
        "ON promo_activations(promo_id, user_id)"
    )
    _add_column(conn, "users", "next_discount_percent INTEGER DEFAULT 0")
    logger.info("Миграция v31 применена: промокоды (promo_codes, promo_activations)")


def migration_32(conn):
    """
    Миграция v32: платёжные методы CryptoBot (Crypto Pay API) и Heleket.

    - Настройки cryptobot_* и heleket_*
    - payments.provider_invoice_id — универсальный ID инвойса провайдера
    - Кнопки btn_pay_cryptobot / btn_pay_heleket на странице prepayment
    """
    for key, value in [
        ('cryptobot_enabled', '0'),
        ('cryptobot_token', ''),
    ...
    _add_column(conn, "payments", "provider_invoice_id TEXT")
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_payments_provider_invoice "
        "ON payments(provider_invoice_id)"
    )

    # Кнопки в дефолтную раскладку prepayment (перед btn_back_main)
    cursor = conn.execute("SELECT buttons_default FROM pages WHERE page_key = 'prepayment'")
    row = cursor.fetchone()
    if row:
        try:
        ...
        existing_ids = {b.get('id') for b in buttons if isinstance(b, dict)}
        new_buttons = [
            ("btn_pay_cryptobot", "🤖 CryptoBot (USDT/TON)"),
            ("btn_pay_heleket", "🧿 Heleket (крипта)"),
        ]
        changed = False
        for btn_id, label in new_buttons:
            if btn_id in existing_ids:
                continue
            max_row = max((b.get('row', 0) for b in buttons if isinstance(b, dict)), default=-1)
            new_row = max_row + 1
            for b in buttons:
                if isinstance(b, dict) and b.get('id') == 'btn_back_main':
                    ...
                    break
            buttons.append({
                "id": btn_id, "label": label, "color": "primary",
                "row": new_row, "col": 0, "is_hidden": False,
    ...
    logger.info("Миграция v32 применена: CryptoBot + Heleket")


def migration_33(conn):
    """
    Миграция v33: UTM/рекламные кампании.

    - campaigns: рекламные ссылки /start ad_{code}
    - users.campaign_id — источник регистрации
    """
    conn.execute("""
    ...
    """)
    _add_column(conn, "users", "campaign_id INTEGER REFERENCES campaigns(id)")
    logger.info("Миграция v33 применена: UTM-кампании")


def migration_34(conn):
    """
    Миграция v34: кнопка «Промокод» на главной странице.

    Добавляет btn_promo в дефолтную раскладку страницы main
    (internal-действие cmd_promo).
    """
    cursor = conn.execute("SELECT buttons_default FROM pages WHERE page_key = 'main'")
    row = cursor.fetchone()
    if row:
        try:
            buttons = json.loads(row['buttons_default'])
        ...
        existing_ids = {b.get('id') for b in buttons if isinstance(b, dict)}
        if 'btn_promo' not in existing_ids:
            max_row = max((b.get('row', 0) for b in buttons if isinstance(b, dict)), default=-1)
            buttons.append({
                "id": "btn_promo",
                "label": "🎟️ Промокод",
                ...
                "action_value": "cmd_promo",
            })
            conn.execute(
                "UPDATE pages SET buttons_default = ? WHERE page_key = 'main'",
                (json.dumps(buttons, ensure_ascii=False),)
            )
            logger.info("Кнопка btn_promo добавлена в дефолтную раскладку main")

    logger.info("Миграция v34 применена: кнопка «Промокод»")


def migration_35(conn):
    """
    Миграция v35: post-pay идемпотентность и partial balance.

    - payments.balance_to_deduct — часть суммы к списанию с баланса при доплате
    - payments.referral_paid_at / user_notified_at — once-флаги post-pay
    - setting webhook_postpay_v2 (default 0) — feature-flag нового webhook flow
    """
    _add_column(conn, "payments", "balance_to_deduct INTEGER NOT NULL DEFAULT 0")
    _add_column(conn, "payments", "referral_paid_at DATETIME")
    _add_column(conn, "payments", "user_notified_at DATETIME")
    conn.execute(
        "INSERT OR IGNORE INTO settings (key, value) VALUES (?, ?)",
        ('webhook_postpay_v2', '0')
    )
    logger.info("Миграция v35 применена: balance_to_deduct / referral_paid_at / user_notified_at")


def migration_36(conn):
    """
    Миграция v36: webhook outbox + reliability settings.
    """
    conn.execute("""
    ...
    """)
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_webhook_outbox_pending ON webhook_outbox(status, attempts)"
    )
    for key, val in (
        ('redis_fsm_url', ''),
        ('webhook_outbox_enabled', '1'),
    ):
        conn.execute(
            "INSERT OR IGNORE INTO settings (key, value) VALUES (?, ?)",
            (key, val),
        )
    logger.info("Миграция v36 применена: webhook_outbox")


def migration_37(conn: sqlite3.Connection) -> None:
    """
    Убирает стороннюю Telegraph-инструкцию из страницы help и ставит
    нейтральный гайд OpenGate без ссылок на чужие проекты и авторов.
    Обновляет text_default и text_custom (если там ещё был telegra.ph).
    """
    new_help = (
        "🔐 <b>Как подключить VPN</b>\n\n"
        "1. Купите ключ в разделе «Купить ключ» или откройте уже купленный в «Мои ключи».\n"
    ...
    conn.execute(
        "UPDATE pages SET text_default = ? WHERE page_key = 'help'",
        (new_help,),
    )
    # Если админ не кастомизировал help, либо в кастоме осталась чужая ссылка — чистим.
    row = conn.execute(
        "SELECT text_custom FROM pages WHERE page_key = 'help'"
    ).fetchone()
    custom = (row["text_custom"] if row else None) or ""
    if (not custom.strip()) or ("telegra.ph/Kak-nastroit-VPN" in custom):
        conn.execute(
            "UPDATE pages SET text_custom = ? WHERE page_key = 'help'",
            (new_help,),
        )
    logger.info("Миграция v37 применена: help без сторонних ссылок")
