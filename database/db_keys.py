...
__all__ = [
    ...
    'ensure_page_token',
    'get_key_by_page_token',
    'set_key_auto_renew',
    'get_auto_renewable_keys',
    'get_link_reset_at',
    'mark_link_reset',
    'set_key_access_password',
]
    ...
    with get_db() as conn:
        cursor = conn.execute("""
            SELECT 
                ...
                s.panel_type, s.public_host, s.extra_config,
...
def add_days_to_first_active_key(user_id: int, days: int) -> Optional[int]:
    """
    Добавить дни к первому активному ключу пользователя.

    Returns:
        key_id если успешно, иначе None
    """
    with get_db() as conn:
        cursor = conn.execute("""
...
# ============================================================================
# Subscription page / автопродление / сброс ссылки
# ============================================================================

def ensure_page_token(key_id: int) -> Optional[str]:
    """
    Возвращает page_token ключа, генерируя его при первом обращении.

    Токен независим от sub_id панели — его можно ревокать (mark_link_reset)
    без пересоздания клиента на панели.
    """
    with get_db() as conn:
        cursor = conn.execute("SELECT page_token FROM vpn_keys WHERE id = ?", (key_id,))
        row = cursor.fetchone()
        if not row:
            return None
        if row['page_token']:
        ...
        conn.execute("UPDATE vpn_keys SET page_token = ? WHERE id = ?", (token, key_id))
        return token


def get_key_by_page_token(token: str) -> Optional[Dict[str, Any]]:
    """Ключ (с данными тарифа и пользователя) по токену subscription page."""
    if not token:
        return None
    with get_db() as conn:
        cursor = conn.execute("""
            SELECT vk.*, t.name as tariff_name, t.duration_days,
                   u.telegram_id, s.name as server_name
            FROM vpn_keys vk
            LEFT JOIN tariffs t ON vk.tariff_id = t.id
            LEFT JOIN users u ON vk.user_id = u.id
            LEFT JOIN servers s ON vk.server_id = s.id
...
def set_key_auto_renew(key_id: int, enabled: bool) -> bool:
    """Включает/выключает автопродление ключа с баланса."""
    with get_db() as conn:
        cursor = conn.execute(
            "UPDATE vpn_keys SET auto_renew = ? WHERE id = ?",
            (1 if enabled else 0, key_id)
        )
        return cursor.rowcount > 0


def get_auto_renewable_keys(days_before: int = 1) -> List[Dict[str, Any]]:
    """
    Ключи с auto_renew=1, истекающие в ближайшие days_before дней.

    Возвращает ключ + тариф + владельца (для списания с баланса).
    """
    with get_db() as conn:
        cursor = conn.execute("""
            SELECT vk.*, t.name as tariff_name, t.duration_days, t.price_rub,
                   t.price_cents, t.traffic_limit_gb,
                   u.telegram_id, u.personal_balance
            FROM vpn_keys vk
...
def get_link_reset_at(key_id: int) -> Optional[str]:
    """Время последнего сброса ссылки (для cooldown)."""
    with get_db() as conn:
        cursor = conn.execute("SELECT link_reset_at FROM vpn_keys WHERE id = ?", (key_id,))
        row = cursor.fetchone()
        return row['link_reset_at'] if row else None


def mark_link_reset(key_id: int, new_page_token: Optional[str] = None) -> None:
    """Фиксирует сброс ссылки; опционально заменяет page_token."""
    with get_db() as conn:
        if new_page_token:
            conn.execute(
                "UPDATE vpn_keys SET link_reset_at = CURRENT_TIMESTAMP, page_token = ? WHERE id = ?",
                (new_page_token, key_id)
            )
        else:
            conn.execute(
                "UPDATE vpn_keys SET link_reset_at = CURRENT_TIMESTAMP WHERE id = ?",
                (key_id,)
            )


def set_key_access_password(key_id: int, password: str) -> None:
    """Сохраняет пароль доступа (naive/mieru)."""
    with get_db() as conn:
        conn.execute(
            "UPDATE vpn_keys SET access_password = ? WHERE id = ?",
            (password, key_id)
        )
